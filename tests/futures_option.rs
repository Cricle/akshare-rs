...
async fn test_futures_main_sina() {
    ...
    let result = client.futures_main_sina("nf_AG0", 10).await;
    assert!(result.is_ok());
}
...
async fn test_futures_zh_minute_sina() {
    ...
    mock_any_get_text(&server, ".*", jsonp).await;
    let client = mock_client(&server);
    let result = client.futures_zh_minute_sina("RB0", "5").await;
    assert!(result.is_ok());
}
...
async fn test_futures_zh_daily_sina() {
    ...
    mock_any_get_text(&server, ".*", jsonp).await;
    let client = mock_client(&server);
    let result = client.futures_zh_daily_sina("RB0").await;
    assert!(result.is_ok());
}
...
async fn test_futures_hist_em() {
    ...
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client.futures_hist_em("rb2405", "daily", "2024-01-01", "2024-01-31").await;
    assert!(result.is_ok());
}
...
async fn test_futures_main_sina_derivative() {
    ...
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    // This delegates to futures_main_sina via derivative.rs
    let result = client.futures_main_sina("nf_AG0", 10).await;
...
async fn test_futures_dce_position_rank_other() {
    ...
    mock_any_post(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client.futures_dce_position_rank_other("20240102", "a").await;
    assert!(result.is_ok());
}
...
async fn test_get_roll_yield_bar() {
    ...
    mock_any_get_text(&server, ".*", "").await;
    let client = mock_client(&server);
    let result = client.get_roll_yield_bar("20240102", None, None, None).await;
    assert!(result.is_ok());
}
...
async fn test_option_sse_list_sina() {
    ...
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client.option_sse_list_sina("50ETF", "SH").await;
    assert!(result.is_ok());
}
...
async fn test_option_sse_expire_day_sina() {
    ...
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client.option_sse_expire_day_sina("202401", "50ETF", "SH").await;
    assert!(result.is_ok());
}
...
async fn test_option_sse_spot_price_sina() {
    ...
    let values = (0..42).map(|i| format!("{}", i)).collect::<Vec<_>>().join(",");
...
async fn test_option_sse_underlying_spot_price_sina() {
    ...
    let values = (0..33).map(|i| format!("{}", i)).collect::<Vec<_>>().join(",");
...
async fn test_option_sse_greeks_sina() {
    ...
    let values = (0..13).map(|i| format!("{}", i)).collect::<Vec<_>>().join(",");
...
async fn test_option_cffex_sina() {
    let server = MockServer::start().await;
    // option_cffex_sz50_list_sina parses HTML for id="option_symbol" and id="option_suffix"
    ...
    let client = mock_client(&server);
    // option_cffex_sina -> option_cffex_sz50_list_sina
...
async fn test_option_commodity_sina() {
    let server = MockServer::start().await;
    // option_commodity_contract_sina makes two HTML requests:
    ...
    mock_any_get_text(&server, ".*", html).await;
    let client = mock_client(&server);
    // option_commodity_sina -> option_commodity_contract_sina
    let result = client.option_commodity_contract_sina("m").await;
    assert!(result.is_ok());
}
...
async fn test_option_analysis_em() {
    let server = MockServer::start().await;
    // option_analysis_em -> option_premium_analysis_em
    let body = serde_json::json!({
        "rc": 0,
        "data": {
            "total": 1,
...
async fn test_option_lhb_em() {
    ...
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client.option_lhb_em("510050", "期权交易情况-认购交易量", "20240102").await;
    let _ = result;
}
