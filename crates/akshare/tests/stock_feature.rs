...
async fn test_stock_dzjy_mrtj() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "BLOCKTRADE_NUM": 5, "BLOCKTRADE_VOLUME": 100000.0, "BLOCKTRADE_AMT": 1000000.0,
        "PREMIUM_RATIO": 0.05, "FREE_MARKET_CAP": 5000000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_dzjy_hygtj() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({
        "INDUSTRY": "Bank", "BLOCKTRADE_NUM": 5, "BLOCKTRADE_VOLUME": 100000.0,
        "BLOCKTRADE_AMT": 1000000.0, "PREMIUM_RATIO": 0.05
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_dzjy_hyyybtj() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "BLOCKTRADE_VOLUME": 100000.0, "BLOCKTRADE_AMT": 1000000.0, "PREMIUM_RATIO": 0.05
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_dzjy_yybph() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({
        "OPERATEDEPT_NAME": "Branch", "BUY_NUM": 5, "BUY_AMT": 1000000.0,
        "SELL_NUM": 3, "SELL_AMT": 500000.0, "NET_AMT": 500000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_gpzy_individual_pledge_ratio_detail_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "PLEDGE_SHARES": 100000.0, "HOLDING_RATIO": 0.5, "TOTAL_RATIO": 0.3,
...
async fn test_stock_gpzy_industry_data_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "PLEDGE_TOTAL_NUM": 50, "TOTAL_PLEDGE_SHARES": 1000000.0, "PLEDGE_TOTAL_MARKETCAP": 5000000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_info_change_name() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({"OLD_NAME": "A", "NEW_NAME": "B"})]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_sector_fund_flow_summary() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({"test": true})]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_report_disclosure() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({"NOTICE_DATE": "2024-01-01"})]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_share_hold_change_bse() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({"SECURITY_CODE": "000001"})]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_sy_hy_em() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({
        "INDUSTRY": "Bank", "PE_RATIO": 15.0, "COMPANY_COUNT": 10, "TOTAL_MARKET_CAP": 1000000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_value_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "TOTAL_MARKET_CAP": 1000000.0, "NOTLIMITED_MARKETCAP_A": 500000.0,
        "TOTAL_SHARES": 100000.0, "FREE_SHARES_A": 50000.0,
...
async fn test_stock_yzxdr_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "HOLDNUM": 100000.0, "HOLDRATIO": 0.5, "HOLDNUMCHANGE": 1000.0,
...
async fn test_stock_zdhtmx_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "AMOUNTS": 1000000.0, "DIM_RDATE": "2024-01-01"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_hsgt_hold_stock_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        "SECURITY_CODE": "000001", "SECURITY_NAME_ABBR": "Test", "CLOSE_PRICE": 10.0,
        "CHANGE_RATE": 1.5, "HOLD_SHARES": 100000.0, "HOLD_MARKET_CAP": 1000000.0,
...
async fn test_stock_hsgt_stock_statistics_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "HOLD_MARKET_CAP": 1000000.0, "HOLD_SHARES": 100000.0,
...
async fn test_stock_hsgt_institution_statistics_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "HOLD_MARKET_CAP": 1000000.0, "HOLD_SHARES": 100000.0, "HOLD_NUM": 50
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client
...
async fn test_stock_hsgt_board_rank_em() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({
        "BOARD_NAME": "Tech", "HOLD_MARKET_CAP": 1000000.0, "HOLD_SHARES": 100000.0, "HOLD_NUM": 50
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_hsgt_individual_detail_em() {
    let server = MockServer::start().await;
    let body = em_datacenter_response(vec![serde_json::json!({
        "TRADE_DATE": "2024-01-01", "HOLD_SHARES": 100000.0, "HOLD_MARKET_CAP": 1000000.0,
        "A_SHARES_RATIO": 0.05, "TOTAL_SHARES_RATIO": 0.03,
        "CLOSE_PRICE": 10.0, "CHANGE_RATE": 1.5
    })]);
    mock_any_get(&server, ".*", body).await;
...
async fn test_stock_lhb_detail_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        "SECURITY_CODE": "000001", "SECURITY_NAME_ABBR": "Test", "TRADE_DATE": "2024-01-01",
        "CLOSE_PRICE": 10.0, "CHANGE_RATE": 1.5, "BILLBOARD_NET_AMT": 1000000.0,
        "BILLBOARD_BUY_AMT": 2000000.0, "BILLBOARD_SELL_AMT": 1000000.0,
        "BILLBOARD_DEAL_AMT": 3000000.0, "ACCUM_AMOUNT": 50000000.0,
        "DEAL_NET_RATIO": 0.02, "DEAL_AMOUNT_RATIO": 0.06, "TURNOVERRATE": 2.0,
        "FREE_MARKET_CAP": 5000000.0, "EXPLANATION": "涨停"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_lhb_stock_statistic_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "BILLBOARD_NET_AMT": 1000000.0, "BILLBOARD_BUY_AMT": 2000000.0,
        "BILLBOARD_SELL_AMT": 1000000.0, "AMOUNT": 3000000.0,
        "BUY_TIMES": 3, "SELL_TIMES": 2, "ORG_BUY_NET_AMT": 500000.0,
        "ORG_BUY_AMT": 1000000.0, "ORG_SELL_AMT": 500000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_lhb_jgmmtj_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "ORG_BUY_AMT": 1000000.0, "ORG_SELL_AMT": 500000.0, "NET_BUY_AMT": 500000.0,
        "ACCUM_AMOUNT": 50000000.0, "NET_BUY_RATIO": 0.01, "TURNOVERRATE": 2.0,
        "FREE_MARKET_CAP": 5000000.0, "EXPLANATION": "涨停"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_lhb_jgstatistic_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        "SECURITY_CODE": "000001", "SECURITY_NAME_ABBR": "Test",
        "CLOSE_PRICE": 10.0, "CHANGE_RATE": 1.5, "AMOUNT": 3000000.0,
        "ONLIST_TIMES": 5, "BUY_AMT": 1000000.0, "BUY_TIMES": 3,
        "SELL_AMT": 500000.0, "SELL_TIMES": 2, "NET_BUY_AMT": 500000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_lhb_hyyyb_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "BUY_NUM": 5, "SELL_NUM": 3, "BUY_AMT": 1000000.0,
        "SELL_AMT": 500000.0, "TOTAL_NETAMT": 500000.0,
...
async fn test_stock_lhb_stock_detail_date_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        "TRADE_DATE": "2024-01-01", "CLOSE_PRICE": 10.0, "CHANGE_RATE": 1.5,
        "EXPLANATION": "涨停", "BILLBOARD_NET_AMT": 1000000.0,
        "BILLBOARD_BUY_AMT": 2000000.0, "BILLBOARD_SELL_AMT": 1000000.0
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_lhb_stock_detail_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "BUY": 1000000.0, "SELL": 500000.0, "NET": 500000.0, "EXPLANATION": "涨停"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
    let result = client
...
async fn test_stock_lhb_yyb_detail_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        "TRADE_DATE": "2024-01-01", "SECURITY_CODE": "000001", "SECURITY_NAME_ABBR": "Test",
        "CLOSE_PRICE": 10.0, "CHANGE_RATE": 1.5, "BUY": 1000000.0,
        "SELL": 500000.0, "NET": 500000.0, "EXPLANATION": "涨停"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_gdfx_free_holding_detail_em() {
    ...
    let body = em_datacenter_response(vec![serde_json::json!({
        ...
        "END_DATE": "2024-01-01", "HOLD_NUM": 100000.0, "UPDATE_DATE": "2024-01-15"
    })]);
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
...
async fn test_stock_inner_trade_xq() {
    let server = MockServer::start().await;
    let body = serde_json::json!({"data": {"list": [{"symbol": "SZ000001", "name": "Test", "changedate": "2024-01-01", "changer": "Person", "changecount": 10000.0, "avgprice": 10.5, "holdcount": 100000.0, "relationship": "Director", "position": "CEO"}]}});
    mock_any_get(&server, ".*", body).await;
    let client = mock_client(&server);
