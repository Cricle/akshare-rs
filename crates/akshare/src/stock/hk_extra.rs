...
impl AkShareClient {
    ...
    /// Get HK sector rankings from Eastmoney.
    ///
    /// `sector_type` is "industry" (恒生行业分类, 31 sectors) or "concept" (概念板块, 287 sectors).
    pub async fn hk_sector_rankings(
        &self,
        sector_type: &str,
        limit: usize,
        ...
        let fs = match sector_type {
            ...
            other => {
                return Err(Error::invalid_input(format!(
                    "unsupported HK sector_type: {other}"
                )));
            }
        };
    ...
    /// Get HK sector capital flow from Eastmoney.
    ///
    /// `sector_code` is the Eastmoney HK sector code (e.g., "HK28" for software services).
    pub async fn hk_sector_capital_flow(
        &self,
        sector_code: &str,
        limit: usize,
    ) -> Result<Vec<crate::types::CapitalFlowPoint>> {
        self.eastmoney_sector_capital_flow_by_prefix("201", sector_code, limit)
            .await
    }

    /// Get HK sector constituents from Eastmoney.
    ///
    /// `sector_code` is the Eastmoney HK sector code (e.g., "HK28" for software services).
    /// Returns stocks belonging to that sector based on the `f100` industry field.
    ///
    /// Note: This fetches the full HK stock list and filters client-side by industry name,
    /// since the Eastmoney `b:{code}` API does not support HK sector codes.
    pub async fn hk_sector_constituents(
        &self,
        sector_code: &str,
        limit: usize,
    ) -> Result<Vec<crate::types::SectorConstituent>> {
        // First, get the sector name from the sector code
        let sector_name = self.resolve_hk_sector_name(sector_code).await?;

        // Fetch HK stocks with industry field (f100)
        ...
        if all_items.is_empty() {
            return Err(Error::not_found(format!(
                "no HK stocks found for sector {sector_code} ({sector_name})"
            )));
        }
    ...
    /// Resolve HK sector code to sector name.
    async fn resolve_hk_sector_name(&self, sector_code: &str) -> Result<String> {
        let sectors = self.hk_sector_rankings("industry", 100).await?;
        for s in &sectors {
        ...
        let concept_sectors = self.hk_sector_rankings("concept", 500).await?;
        ...
        Err(Error::not_found(format!(
            "HK sector code {sector_code} not found"
        )))
    }
}
