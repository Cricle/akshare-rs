...
impl AkShareClient {
    ...
    pub async fn fund_graded(&self, limit: usize) -> Result<Vec<FundSnapshot>> {
        ...
        let items = payload
            .get("data").and_then(|d| d.get("diff")).and_then(|d| d.as_array())
    ...
    pub async fn fund_graded_fund_daily_em(&self) -> Result<Vec<serde_json::Value>> {
        let response = self
            ...
            .query(&[
                ("t", "1"), ("lx", "9"), ("letter", ""), ("gsid", "0"),
                ("text", ""), ("sort", "zdf,desc"), ("page", "1,10000"),
                ("dt", "1580914040623"), ("atfc", ""),
            ])
            .send().await.map_err(Error::from)?
            .error_for_status().map_err(Error::from)?;
    ...
    pub async fn fund_graded_fund_info_em(&self, symbol: &str) -> Result<Vec<FundNavHistory>> {
        ...
        let payload: serde_json::Value = response.json().await.map_err(Error::from)?;
        let list = payload
            .get("Data").and_then(|d| d.get("LSJZList")).and_then(|l| l.as_array())
        ...
        if result.is_empty() {
            return Err(Error::not_found(format!("no graded fund data for {symbol}")));
        }
