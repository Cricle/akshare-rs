...
impl AkShareClient {
    ...
    pub async fn fund_report_half_year_cninfo(&self, symbol: &str) -> Result<Vec<serde_json::Value>> {
        Err(Error::decode(format!(
    ...
    pub async fn fund_report_stock_cninfo(&self, _date: &str) -> Result<Vec<serde_json::Value>> {
        ...
        Err(Error::decode("fund_report_stock_cninfo requires JS authentication (CNINFO mcode)"))
    }

    /// Fetch fund industry allocation from CNINFO (Python: fund_report_industry_allocation_cninfo).
    pub async fn fund_report_industry_allocation_cninfo(&self, _date: &str) -> Result<Vec<serde_json::Value>> {
        Err(Error::decode("fund_report_industry_allocation_cninfo requires JS authentication"))
    }
    ...
    pub async fn fund_report_asset_allocation_cninfo(&self) -> Result<Vec<serde_json::Value>> {
        Err(Error::decode("fund_report_asset_allocation_cninfo requires JS authentication"))
    }
}
