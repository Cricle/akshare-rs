...
impl AkShareClient {
    ...
    pub async fn stock_zcfz_em(&self, date: &str) -> Result<Vec<BalanceSheet>> {
        ...
        let data = self.dc_fetch_all("RPT_DMSK_FN_BALANCE", "ALL", &filter, "NOTICE_DATE,SECURITY_CODE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| BalanceSheet {
    ...
    pub async fn stock_lrb_em(&self, date: &str) -> Result<Vec<ProfitSheet>> {
        let date_fmt = fmt_date(date);
        let filter = format!("(SECURITY_TYPE_CODE in (\"058001001\",\"058001008\"))(TRADE_MARKET_CODE!=\"069001017\")(REPORT_DATE='{}')", date_fmt);
        let data = self.dc_fetch_all("RPT_DMSK_FN_INCOME", "ALL", &filter, "NOTICE_DATE,SECURITY_CODE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| ProfitSheet {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            notice_date: json_str_opt(v, "NOTICE_DATE"),
    ...
    pub async fn stock_xjll_em(&self, date: &str) -> Result<Vec<CashFlowSheet>> {
        let date_fmt = fmt_date(date);
        let filter = format!("(SECURITY_TYPE_CODE in (\"058001001\",\"058001008\"))(TRADE_MARKET_CODE!=\"069001017\")(REPORT_DATE='{}')", date_fmt);
        let data = self.dc_fetch_all("RPT_DMSK_FN_CASHFLOW", "ALL", &filter, "NOTICE_DATE,SECURITY_CODE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| CashFlowSheet {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            notice_date: json_str_opt(v, "NOTICE_DATE"),
