...
impl AkShareClient {
    ...
    pub async fn stock_changes_em(&self, symbol: &str) -> Result<Vec<PankouChange>> {
        let data = self.push2ex_fetch("getChangesList", &[
        ...
        let diff = data.get("data").and_then(|d| d.get("details")).and_then(|d| d.as_array()).cloned().unwrap_or_default();
        Ok(diff.iter().filter_map(|v| {
            let s = v.as_str()?;
