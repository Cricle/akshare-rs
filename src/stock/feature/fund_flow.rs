...
impl AkShareClient {
    ...
    /// 东方财富-板块资金流向-历史
    pub async fn stock_sector_fund_flow_hist(&self, symbol: &str) -> Result<Vec<SectorFundFlowHist>> {
        ...
        let klines = json.get("data")
            ...
            .cloned()
            .unwrap_or_default();
        Ok(klines.iter().map(|v| SectorFundFlowHist {
            data: v.clone(),
        }).collect())
    }

    /// 东方财富-板块资金流向-排名
    pub async fn stock_sector_fund_flow_rank(&self, indicator: &str) -> Result<Vec<SectorFundFlowRank>> {
    ...
    pub async fn stock_sector_fund_flow_summary(&self) -> Result<Vec<SectorFundFlowSummary>> {
        ...
        Ok(data.iter().map(|v| SectorFundFlowSummary {
            data: v.clone(),
        }).collect())
    }
    ...
    pub async fn stock_main_fund_flow(&self, symbol: &str) -> Result<Vec<MainFundFlow>> {
        ...
        let klines = json.get("data")
            ...
            .cloned()
            .unwrap_or_default();
        Ok(klines.iter().map(|v| MainFundFlow {
            data: v.clone(),
        }).collect())
    }
    ...
    pub async fn stock_market_fund_flow(&self) -> Result<Vec<MarketFundFlow>> {
        ...
        Ok(data.iter().map(|v| MarketFundFlow {
            data: v.clone(),
        }).collect())
    }

    /// 东方财富-概念资金流向-历史
    pub async fn stock_concept_fund_flow_hist(&self, symbol: &str) -> Result<Vec<ConceptFundFlowHist>> {
        let url = "https://push2his.eastmoney.com/api/qt/stock/fflow/daykline/get";
        let resp = self.get(url)
            .query(&[
                ("lmt", "0"),
        ...
        let klines = json.get("data")
            ...
            .cloned()
            .unwrap_or_default();
        Ok(klines.iter().map(|v| ConceptFundFlowHist {
            data: v.clone(),
        }).collect())
    }

    /// Generic sector fund flow fetch via Eastmoney.
    async fn sector_fund_flow_fetch(&self, _kind: &str, _indicator: &str) -> Result<Vec<SectorFundFlowRank>> {
        let data = self.cistock_fetch(
            "m:90+t:2",
            "f2,f3,f12,f14,f62,f184,f66,f69,f72,f75,f78,f164,f166",
            "5000",
        ...
        Ok(data.iter().map(|v| SectorFundFlowRank {
            name: json_str(v, "f14"),
            change_pct: json_f64_opt(v, "f3"),
            inflow: json_f64_opt(v, "f62"),
            outflow: None,
