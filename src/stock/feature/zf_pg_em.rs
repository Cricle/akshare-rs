...
impl AkShareClient {
    ...
    pub async fn stock_qbzf_em(&self) -> Result<Vec<Qbzf>> {
        let data = self.dc_fetch_all("RPT_SEO_DETAIL", "ALL", "", "ISSUE_DATE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| Qbzf {
    ...
    pub async fn stock_pg_em(&self) -> Result<Vec<Pg>> {
        let data = self.dc_fetch_all("RPT_RIGHTS_ISSUE", "ALL", "", "PLAN_NOTICE_DATE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| Pg {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
