...
impl AkShareClient {
    ...
    pub async fn stock_analyst_rank_em(&self, year: &str) -> Result<Vec<AnalystRank>> {
        ...
        let data = self.dc_fetch_all("RPT_ANALYST_INDEX_RANK", "ALL", &filter,
            "YEAR_YIELD", "-1", 500, 1, &[("distinct", "ANALYST_CODE"), ("limit", "top100")]).await?;
        Ok(data.iter().map(|v| AnalystRank {
    ...
    /// 东方财富分析师详情
    pub async fn stock_analyst_detail_em(&self, analyst_id: &str, indicator: &str) -> Result<Vec<AnalystDetail>> {
        ...
        let data = self.dc_fetch_all(report, cols, &filter,
            "TRADE_DATE", "-1", 500, 1, &[]).await?;
        Ok(data.iter().map(|v| AnalystDetail {
