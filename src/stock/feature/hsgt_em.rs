...
impl AkShareClient {
    ...
    pub async fn stock_hsgt_fund_flow_summary_em(&self) -> Result<Vec<HsgtFundFlowSummary>> {
        ...
        Ok(data.iter().map(|v| HsgtFundFlowSummary {
    ...
    pub async fn stock_hsgt_hold_stock_em(
        ...
        Ok(data.iter().map(|v| HsgtHoldStock {
    ...
    pub async fn stock_hsgt_stock_statistics_em(
        ...
        Ok(data.iter().map(|v| HsgtStockStatistic {
            trade_date: json_str(v, "TRADE_DATE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            code: json_str(v, "SECURITY_CODE"),
            holding_market_cap: json_f64(v, "HOLD_MARKET_CAP"),
    ...
    pub async fn stock_hsgt_institution_statistics_em(
        ...
        Ok(data.iter().map(|v| HsgtInstitutionStatistic {
            trade_date: json_str(v, "TRADE_DATE"),
    ...
    /// 沪深港通历史数据
    pub async fn stock_hsgt_hist_em(
        &self,
        _symbol: &str,
    ) -> Result<Vec<HsgtHist>> {
        let data = self.dc_fetch_all(
    ...
    pub async fn stock_hsgt_board_rank_em(
        ...
        Ok(data.iter().map(|v| HsgtBoardRank {
    ...
    pub async fn stock_hsgt_individual_detail_em(
        ...
        let filter = format!(
            "(SECURITY_CODE=\"{symbol}\")(INTERVAL_TYPE=\"{indicator_type}\")"
        );
        let data = self.dc_fetch_all(
            "RPT_MUTUAL_STOCK_NORTHSTA",
            "ALL",
        ...
        Ok(data.iter().map(|v| HsgtIndividualDetail {
            trade_date: json_str(v, "TRADE_DATE"),
            holding_shares: json_f64(v, "HOLD_SHARES"),
            holding_market_cap: json_f64(v, "HOLD_MARKET_CAP"),
            holding_circulating_ratio: json_f64(v, "A_SHARES_RATIO"),
    ...
    pub async fn stock_hsgt_fund_min_em(&self, symbol: &str) -> Result<Vec<serde_json::Value>> {
        ...
        let url = format!("https://push2.eastmoney.com/api/qt/kamtbs.rtmin/get?fields1=f1,f2,f3,f4&fields2=f51,f52,f53,f54,f55,f56&ut=b2884a393a59ad64002292a3e90d46a5&klt={}", market);
        let resp = self.get(&url).send().await.map_err(Error::from)?
            .error_for_status().map_err(Error::from)?;
        let data: serde_json::Value = resp.json().await.map_err(Error::from)?;
        let items = data.get("data").and_then(|d| d.get("s2n")).and_then(|v| v.as_array()).cloned().unwrap_or_default();
        Ok(items)
    }

    /// 沪深港通持股-个股 (Python: stock_hsgt_individual_em)
    pub async fn stock_hsgt_individual_em(&self, symbol: &str) -> Result<Vec<HsgtIndividualDetail>> {
