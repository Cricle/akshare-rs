...
impl AkShareClient {
    ...
    pub async fn stock_sy_profile_em(&self) -> Result<Vec<SyProfile>> {
        let data = self.dc_fetch_all("RPT_GOODWILL_MARKET", "ALL", "", "REPORT_DATE", "-1", 500, 2, &[]).await?;
        Ok(data.iter().map(|v| SyProfile {
    ...
    pub async fn stock_sy_yq_em(&self, indicator: &str) -> Result<Vec<SyYqDetail>> {
        ...
        let data = self.dc_fetch_all(report_name, "ALL", "", "GOODWILL", "-1", 500, 5, &[]).await?;
        Ok(data.iter().map(|v| SyYqDetail {
    ...
    pub async fn stock_sy_jz_em(&self, symbol: &str) -> Result<Vec<SyJzDetail>> {
        ...
        let data = self.dc_fetch_all("RPT_GOODWILL_IMPAIRMENT", "ALL", &filter, "REPORT_DATE", "-1", 500, 2, &[]).await?;
        Ok(data.iter().map(|v| SyJzDetail {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            report_date: json_str(v, "REPORT_DATE"),
            notice_date: json_str(v, "NOTICE_DATE"),
    ...
    pub async fn stock_sy_em(&self, symbol: &str) -> Result<Vec<SyDetail>> {
        let filter = format!("(SECURITY_CODE=\"{symbol}\")");
        let data = self.dc_fetch_all("RPT_GOODWILL_DETAIL", "ALL", &filter, "REPORT_DATE", "-1", 500, 2, &[]).await?;
        Ok(data.iter().map(|v| SyDetail {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            report_date: json_str(v, "REPORT_DATE"),
            notice_date: json_str(v, "NOTICE_DATE"),
