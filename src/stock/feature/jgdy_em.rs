...
impl AkShareClient {
    ...
    pub async fn stock_jgdy_tj_em(&self, date: &str) -> Result<Vec<JgdyTj>> {
        ...
        let data = self.dc_fetch_all("RPT_ORG_SURVEYNEW", "ALL", &filter, "NOTICE_DATE,SUM,RECEIVE_START_DATE,SECURITY_CODE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| JgdyTj {
    ...
    pub async fn stock_jgdy_detail_em(&self, date: &str) -> Result<Vec<JgdyDetail>> {
        ...
        let data = self.dc_fetch_all("RPT_ORG_SURVEY", "ALL", &filter, "NOTICE_DATE,SECURITY_CODE", "-1", 500, 10, &[]).await?;
        Ok(data.iter().map(|v| JgdyDetail {
            code: json_str(v, "SECURITY_CODE"),
            name: json_str(v, "SECURITY_NAME_ABBR"),
            notice_date: json_str(v, "NOTICE_DATE"),
            receive_date: json_str(v, "RECEIVE_START_DATE"),
