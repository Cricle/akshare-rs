...
impl AkShareClient {
    ...
    pub async fn stock_margin_account_info_em(
        ...
        let data = self.dc_fetch_all("RPTA_WEB_RZRQ_GK", "ALL", &filter, "TRADE_DATE", "-1", 500, 10, &[]).await?;

        Ok(data.iter().map(|v| MarginAccountInfo {
    ...
    pub async fn stock_margin_detail_sse(&self, date: &str) -> Result<Vec<MarginSseDetail>> {
        ...
        Ok(arr.iter().filter_map(|row| {
            let cols = row.as_array()?;
            if cols.len() < 13 { return None; }
    ...
    pub async fn stock_margin_detail_szse(&self, date: &str) -> Result<Vec<MarginSzseDetail>> {
        ...
        Ok(arr.iter().filter_map(|row| {
            ...
            Some(MarginSzseDetail {
                ...
                fin_buy_amount: clean(cols[2].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                fin_balance: clean(cols[3].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_sell_amount: clean(cols[4].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_volume: clean(cols[5].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_balance: clean(cols[6].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                margin_balance: clean(cols[7].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
            })
        }).collect())
    }

    /// 平安-融资融券保证金比例
    pub async fn stock_margin_ratio_pa(&self, symbol: &str, date: &str) -> Result<Vec<MarginRatioPa>> {
        ...
        let arr = json.get("data")
            ...
            .cloned()
            .unwrap_or_default();
        Ok(arr.iter().map(|v| MarginRatioPa {
    ...
    /// 上海证券交易所-融资融券汇总
    pub async fn stock_margin_sse(&self, start_date: &str, end_date: &str) -> Result<Vec<MarginSseSummary>> {
        let url = "https://query.sse.com.cn/marketdata/tradedata/queryMargin.do";
        let resp = self.get(url)
            .query(&[
                ("isPagination", "true"),
    ...
    pub async fn stock_margin_szse(&self, date: &str) -> Result<Vec<MarginSzseSummary>> {
        ...
        Ok(arr.iter().filter_map(|row| {
            ...
            Some(MarginSzseSummary {
                fin_buy_amount: clean(cols[0].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                fin_balance: clean(cols[1].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_sell_amount: clean(cols[2].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_volume: clean(cols[3].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                loan_balance: clean(cols[4].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
                margin_balance: clean(cols[5].as_str().unwrap_or("0")).parse().unwrap_or(0.0),
            })
        }).collect())
    }

    /// 深圳证券交易所-标的证券信息
    pub async fn stock_margin_underlying_info_szse(&self, date: &str) -> Result<Vec<MarginUnderlyingInfoSzse>> {
        let url = "https://www.szse.cn/api/report/ShowReport";
        let formatted_date = fmt_date(date);
        let resp = self.get(url)
            .query(&[
